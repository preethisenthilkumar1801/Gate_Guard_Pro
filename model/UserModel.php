...
<?php
	...
	class User extends DbOperation
	{
		...
		// 	else {
		// 		echo "rejected"; // Print "rejected" if the result is false
		...
		function getAllleaveHistory()
{
    ...
    if ($result) {
        ...
        return $res; // Print "rejected" if the condition is not met
    }
}
		...
		function getHistory()
		{
			...
			$tablesName = "employeeleaveapplicationdetails";
			//database table name
			$conditions = "WHERE lEmployeeCodeNumberWhoApply = '".$_SESSION['officeUserName']."' AND lIsRecomanded = 0";
			$result = $objDbOperation->dbJoinSingleSelect($columnsName,$tablesName,$conditions);
			return $result;
